...
const Internship = () => {
  ...
  const columns = [
    ...
    <button type="button" data-tooltip="Edit Project"  onClick={()=>navigate(`/auth/company/edit-internship/${row._id}`)} className="px-2 py-1 rounded-full focus:outline-none text-white bg-orange-500 hover:bg-orange-600 focus:ring-text-orange-600 font-medium mr-2 mb-2 text-sm">
  ...
  const GetCategoryData = async ()=>{
    try {
      const resp = await authFetch(`/company/intership`);
  ...
  return (
        <>
          {/* --------------All project----------------- */}
          <ProjectComponents data={{path:'/auth/company/upload-internship',setSearchText,columns,categoryData,searchText}}/>
          <ToastContainer />
