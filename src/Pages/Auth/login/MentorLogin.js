...
const MentorLogin = () =>{
  ...
  useEffect(() => {
    let login = localStorage.getItem('mentor-token');
    if(login){
    ...
    <div>
      ...
      <Formik
        ...
        onSubmit={async(values) => {
          ...
          try {
            const res = await authFetch.post('/mentor/login', {
              email:values.email,
              password:encodedPassword
            });
            localStorage.setItem('mentor-token', res.data.token, true)
      ...
      <div className="flex items-center justify-center">
        <div className="text-sm">
          Don't have account?&nbsp;<NavLink to="/mentor-register" className="font-medium text-black hover:text-gray-600">Register</NavLink>
        </div>
      </div>
    </Form>
     )}
