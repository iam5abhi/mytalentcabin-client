...
useEffect(() => {
  let login = window.localStorage.getItem('mentor-token');
  if(login){
  ...
  <div className="w-full max-w-md space-y-8">
    <div>
      ...
      onSubmit={async(values) => {
        ...
        try {
          const res = await authFetch.post("/mentor/signup",{
            name:values.name,
            email:values.email,
      ...
      <div className="text-sm">
        Allready have a account?&nbsp;<NavLink to="/mentor-login" className="font-medium text-black hover:text-gray-600">Login</NavLink>
      </div>
    </div>
    </Form>
     )}
