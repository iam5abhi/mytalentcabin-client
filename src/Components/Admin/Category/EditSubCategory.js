...
export default function EditSubCategory({setOpen,open,GetSubCategoryData,id}) {
  ...
  const SubmitCategoryData = async ()=>{
    try {
      const res = await authFetch.patch(`/admin/subcategory/${id}`,{name:categoryData.title});
  ...
  const GetSingleCategoryData = async ()=>{
    try {
      const res = await authFetch(`/admin/subcategory/${id}`);
                       ...
                       <div className=" max-w-screen-lg mx-auto">
                        <div className=" mt-9 px-4 ">
                            ...
                            <div className="items-center px-3 gap-4">  
                                <label htmlFor="first_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sub-Category <span class="text-red-600">*</span></label>      
